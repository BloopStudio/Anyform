...
/**
 * Détecte un HEIC/HEIF par sa structure ISO-BMFF (même conteneur que MP4) plutôt que par
 * l'extension du fichier : les 4 premiers octets sont la taille de la première box, suivis
 * du tag "ftyp" (octets 4-8) puis de son "major brand" (octets 8-12), qui identifie le
 * sous-format réel. Nécessaire car sharp ne sait pas décoder du HEIC/HEIF nativement — c'est
 * ce test qui déclenche le passage par decodeHeic().
 */
...
// Normalise un format demandé en ligne de commande (peut arriver avec un point de tête, ex.
// ".png", et/ou en majuscules) vers l'identifiant interne utilisé partout dans le code, où
// "jpeg" et "jpg" sont toujours traités comme un seul et même format.
function normalizeFormat(format) {
  const f = String(format || '').toLowerCase().replace(/^\./, '');
  return f === 'jpeg' ? 'jpg' : f;
}

// Détecte un SVG par son contenu (balise <svg>, éventuellement précédée d'une déclaration
// XML) plutôt que par extension — utile car un buffer peut arriver ici après un decodeHeic
// ou sans extension fiable. Ne lit que les 1000 premiers octets : largement suffisant pour
// atteindre la balise <svg> même après un DOCTYPE ou des commentaires en tête de fichier.
...
async function compressImage(inputBuffer, sourceExt, level = 'medium') {
  ...
  if (!isLossy) {
    ...
    if (scale < 1) {
      // Réutilise le pipeline déjà créé pour lire les métadonnées plutôt que de recréer une
      // seconde instance sharp (et redécoder l'en-tête du buffer une deuxième fois).
...
// quadratique — un PDF de quelques dizaines de Mo peut alors bloquer `info`/`-c` plusieurs
// dizaines de secondes). Ici `lastIndex` saute directement après chaque "endobj" trouvé, donc
// chaque octet du fichier n'est examiné qu'une fois au total (coût linéaire). Utilisé par
// inspectPdf (lib/inspect.js) et compressPdf ci-dessous.
...
async function compressPdf(inputBuffer, level = 'medium') {
  ...
  for (const obj of objects) {
    if (obj.gen !== '0') return inputBuffer; // génération non nulle : cas rare, non géré
