...
function runFfmpeg(args) {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.on('close', (code) => {
      ...
      reject(new Error(t('media.errFfmpegFailed', { code, stderr: stderr.split('\n').slice(-15).join('\n') })));
    });
  });
}
...
function compressVideo(inputPath, outputPath, ext, level = 'medium') {
  ...
  if (!buildArgs) {
    return Promise.reject(new Error(t('media.errVideoCompressUnsupported', { ext })));
  }
...
function compressAudio(inputPath, outputPath, ext, level = 'medium') {
  ...
  if (!buildArgs) {
    return Promise.reject(new Error(t('media.errAudioCompressUnsupported', { ext })));
  }
  return runFfmpeg(['-i', inputPath, ...buildArgs(level), outputPath]);
}
