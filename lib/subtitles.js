...
// Formate un nombre de secondes en timecode SRT ("HH:MM:SS,mmm").
function formatSrtTime(seconds) {
  const { h, m, s, ms } = splitTimeMs(seconds);
  return `${pad(h, 2)}:${pad(m, 2)}:${pad(s, 2)},${pad(ms, 3)}`;
}

// Formate un nombre de secondes en timecode VTT ("HH:MM:SS.mmm" — même précision que SRT,
// seul le séparateur des millisecondes change).
...
// Choisit le bon parseur selon le format source : ASS/SSA a une structure de fichier
// entièrement différente (sections [Script Info]/[V4+ Styles]/[Events]) de SRT/VTT, qui
// eux partagent le même parseur (voir parseSrtOrVtt).
...
// Choisit le bon écrivain selon le format cible, ou lève une erreur si `targetExt` n'est
// pas l'un des formats de SUBTITLE_FORMATS.
function writeSubtitle(cues, targetExt) {
  ...
  throw new Error(t('subtitles.errFormatUnsupported', { format: targetExt }));
}
...
function convertSubtitle(text, sourceExt, targetExt) {
  ...
  if (cues.length === 0) {
    throw new Error(t('subtitles.errUnrecognized'));
  }

  return writeSubtitle(cues, targetExt);
}

module.exports = { convertSubtitle, parseSubtitle, SUBTITLE_FORMATS };
